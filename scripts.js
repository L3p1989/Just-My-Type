...
$("body").keypress(function(e) {
  if (sentenceCount == 4 && keyPress == 48) {
    ...
    var score = numberOfWords / minutes;
    ...
    $("#feedback").append(
      '<img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQUrCET8arubXWUHRwAT4mg5vj6ZylmC3_ongmidqgoP9U2UYYa" id="end-no">'
    ); //appends img to `#feedback` for no
    $("#feedback").prepend(
      '<img src="https://t3.ftcdn.net/jpg/00/88/93/72/240_F_88937261_fZzx2GOTcz0ijLz1tuVvZ3MlEH6sfP4B.jpg" id="end-yes">'
    ); //prepends img to `#feedback` for yes
    ...
    return $("#target-letter").text(
      "Your score is " +
        score +
        " words per minute! Would you like to try again?"
    ); //endgame message
  } else if (
    String.fromCharCode(e.which) == sentences[sentenceCount][keyPress]
  ) {
    keyPress++;

    $("#yellow-block").animate({ left: "+=17.5px" }, 0.1); //animates the yellow highlight

    $("#target-letter").text(sentences[sentenceCount][keyPress]); //changes text of `#target-letter` as keys are pressed

    $("#stop-go").attr(
      "src",
      "https://t3.ftcdn.net/jpg/00/88/93/72/240_F_88937261_fZzx2GOTcz0ijLz1tuVvZ3MlEH6sfP4B.jpg"
    ); //posts check img to `#feedback`
  } else {
    numberOfMistakes++;
    $("#stop-go").attr(
      "src",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQUrCET8arubXWUHRwAT4mg5vj6ZylmC3_ongmidqgoP9U2UYYa"
    ); //posts red X img to `#feedback`
  }
  $("#" + e.which).animate(
    {
      padding: "26px",
  ...
  if (
    sentences[sentenceCount][keyPress] == [0][47] ||
    [1][46] ||
    [2][47] ||
    [3][47]
  ) {
    ...
    $("#yellow-block").animate({ left: "15px" }, 0.1); //resets `#yellow-block` position
...
}); //this highlights the pressed key for a moment, and tracks the correct key was pressed, as well as moving the yellow cursor across the screen
